...
class KNN(BinaryClassifier):
    ...
    def euclidean_distance(self,X,xhat):
        return (sqrt(((X - xhat)*(X - xhat)).sum()))
    ...
    def predict(self, X):
        ...
        if self.trY.size == 0:
            ...
            ### TODO: YOUR CODE HERE
            eucl_dist = 0
            count = 0                  # keeping track of the neighbours
            pos = 0
            neg = 0
            dist = []
            for xhat in X_true:
                eucl_dist = self.euclidean_distance(X,xhat)                                   #sqrt(((X - xhat)*(X - xhat)).sum())               
                dist.append((eucl_dist,count))
                count = count + 1
            dist.sort()
            for i,j in dist[0:K]:
                if Y[j] == 1:
